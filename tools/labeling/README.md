...
2. Generate task JSON:
   ```powershell
   .\generate-label-studio-tasks.ps1 -DatasetPath "data\labeled\LEGO-Train-Traffic-Lights"
   ```
...
```
labeling/
├── start-label-studio.bat          # Launch Label Studio
├── label_studio_config.xml         # Annotation config
├── generate-label-studio-tasks.ps1 # Generate task JSON
